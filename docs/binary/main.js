...
function setTable(top2, probs) {
    //loop over the predictions 
    for (var i = 0; i < top2.length; i++) {
        let sym = document.getElementById('sym' + (i + 1))
        let prob = document.getElementById('prob' + (i + 1))
        sym.innerHTML = top2[i]
...
function getFrame() {
        ...
        //get the prediction 
        const prediction = model.predict(preprocess(imgData)).dataSync()
        const pred = [1-prediction,prediction]
